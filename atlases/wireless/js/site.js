/* =========================================================================
   site.js — shared project chrome for the Wireless Atlas: the common header,
   the EN/UK language switch, the bilingual search, and the i18n engine.
   Include AFTER catalog.js. Add <div id="site-header"></div> and set
   <body data-page="home|analog|keying|…"> on each page.
   ========================================================================= */
(function(){
  const LS_KEY='wireless-lang';
  ...
  const THEME_KEY='wireless-theme';
  let THEME = localStorage.getItem(THEME_KEY) || 'dark';
  if(THEME!=='dark' && THEME!=='light') THEME='dark';
  document.documentElement.setAttribute('data-theme', THEME);
  /* the glyph shown is the theme you switch TO: sun ⇒ go light, moon ⇒ go dark */
  ...
  const NAVPAGES = [
    ...
    {p:'analog',      href:'analog.html',       en:'Analog',       uk:'Аналогові'},
    {p:'keying',      href:'keying.html',        en:'Keying',       uk:'Маніпуляції'},
    {p:'qam',         href:'qam.html',           en:'QAM',          uk:'QAM'},
    {p:'spread',      href:'spread.html',        en:'Spread',       uk:'Розширення'},
    {p:'multicarrier',href:'multicarrier.html',  en:'Multi-Carrier',uk:'Багаточаст.'},
    {p:'codes',       href:'codes.html',         en:'Block Codes',  uk:'Блокові коди'},
    {p:'iterative',   href:'iterative.html',     en:'Iterative FEC',uk:'Ітеративні'},
    {p:'standards',   href:'standards.html',     en:'Standards',    uk:'Стандарти'}
  ];

  /* flat searchable index of every technology */
  const INDEX = [];
  CATALOG.forEach(sec=>{
    sec.algos.forEach(a=>{
      INDEX.push({
  ...
  function applyI18n(root){
    (root||document).querySelectorAll('[data-en]').forEach(el=>{
      ...
      // Attribute values may contain inline markup (<b>, <br>), so swap via innerHTML.
  ...
  function buildHeader(){
    ...
    holder.innerHTML = `
      <div class="site-header">
        <div class="bar">
          <a class="brand" href="index.html">Wireless<span>·</span>Atlas</a>
  ...
  function cxChip(id){
    ...
    return c?' <span class="cx c'+c[1]+'" title="receiver / DSP complexity">'+c[0]+'</span>':'';
  }
